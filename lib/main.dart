...
class _CreditScoreScreenState extends State<CreditScoreScreen> {
  ...
  Future<void> _initWeb3() async {
    ...
    try {
      String abiJson = await rootBundle.loadString(
        'assets/CreditScoreABI.json',
      );
  ...
  Future<void> _addOrUpdateBorrower() async {
    ...
    try {
      ...
      setState(() {
        _statusMessage =
            'Transaction sent! Hash: $txHash. Please wait a moment, then fetch details.';
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _fetchBorrowerDetails() async {
    if (_nid.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter NID to fetch details.')),
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      setState(() {
        _statusMessage =
            'Error fetching details. The borrower may not exist yet.';
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _buildScoreSlider(
                'Account Balance',
                _accountBalanceScore,
                (val) => setState(() => _accountBalanceScore = val),
              ),
              _buildScoreSlider(
                'Payment History',
                _paymentHistoryScore,
                (val) => setState(() => _paymentHistoryScore = val),
              ),
              _buildScoreSlider(
                'Total Transactions',
                _totalTransactionsScore,
                (val) => setState(() => _totalTransactionsScore = val),
              ),
              _buildScoreSlider(
                'Remaining Loan',
                _totalRemainingLoanScore,
                (val) => setState(() => _totalRemainingLoanScore = val),
              ),
              _buildScoreSlider(
                'Credit Age',
                _creditAgeScore,
                (val) => setState(() => _creditAgeScore = val),
              ),
              _buildScoreSlider(
                'Professional Risk',
                _professionalRiskFactorScore,
                (val) => setState(() => _professionalRiskFactorScore = val),
              ),
